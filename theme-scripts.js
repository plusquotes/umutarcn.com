...
(function ($) {
	...
	jQuery(window).scroll(function() {
		if(jQuery(".sidebar-fixed").length){
			...
			if(jQuery(window).scrollTop()+100 >= fixedSidebar.offset().top){
				var newtop = parseInt(jQuery(window).scrollTop())-parseInt(fixedSidebar.offset().top)+100,
					v1 = parseInt(fixedSidebar.height()),
					v2 = parseInt(newtop)+parseInt(fixedSidebar.offset().top);

				if(jQuery(".footer").offset().top <= (v1+v2)){
